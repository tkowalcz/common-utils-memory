...
class FixedSafeByteBufferMemory extends AbstractByteBufferMemory implements BytesWrappingCapableMemory {

    protected byte[] memory;

    FixedSafeByteBufferMemory(int size) {
        memory = new byte[size];
        capacity = size;
    }

    FixedSafeByteBufferMemory(byte[] buffer) {
        memory = buffer;
