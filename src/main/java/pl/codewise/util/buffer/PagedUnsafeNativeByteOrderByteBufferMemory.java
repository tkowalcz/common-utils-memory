...
import static pl.codewise.util.buffer.MemoryAccess.getNativeByteOrderLongUnsafe;
import static pl.codewise.util.buffer.MemoryAccess.setNativeByteOrderLongUnsafe;

public class PagedUnsafeNativeByteOrderByteBufferMemory extends AbstractPagedByteBufferMemory {
    ...
    public PagedUnsafeNativeByteOrderByteBufferMemory(int pageCntGrow, int pageSizeBits, int initialCapacity) {
    ...
    public PagedUnsafeNativeByteOrderByteBufferMemory() {
        super();
        pageIndexMask = ~ ((long) pageAddrMask);
        pageIndexScaleBits = pageSizeBits - ARRAY_OBJECT_INDEX_SCALE_BITS;
    }

    public PagedUnsafeNativeByteOrderByteBufferMemory(int size) {
        super(size);
        pageIndexMask = ~ ((long) pageAddrMask);
        pageIndexScaleBits = pageSizeBits - ARRAY_OBJECT_INDEX_SCALE_BITS;
    }
    ...
    public void putChar(int index, char value) {
        ...
        if (pageAddr < pageAddrMask) {
            ...
            Object page0 = getObjectUnsafe(memory, pageOffset);
            setByteUnsafe(page0, ARRAY_BYTE_BASE_OFFSET + pageAddr, getByte0(value));
            Object page1 = getObjectUnsafe(memory, pageOffset + ARRAY_OBJECT_INDEX_SCALE);
            setByteUnsafe(page1, ARRAY_BYTE_BASE_OFFSET, getByte1(value));
        }
    }
    ...
    public int getInt(int index) {
        ...
        if (memoryOffset == lastByteMemoryOffset) {
            ...
            return asInt(i0, i1, (pageAddr & 0x3) << 3);
        }
    }
    ...
    public void putInt(int index, int value) {
        ...
        if (memoryOffset == lastByteMemoryOffset) {
            ...
            setByteUnsafe(page, pageOffset++, getByte0(value));
            ...
            setByteUnsafe(page, pageOffset++, getByte1(value));
            if (pageOffset > lastPageOffset) {
                memoryOffset += ARRAY_OBJECT_INDEX_SCALE;
                page = getObjectUnsafe(memory, memoryOffset);
                pageOffset = ARRAY_BYTE_BASE_OFFSET;
            }
            setByteUnsafe(page, pageOffset++, getByte2(value));
            if (pageOffset > lastPageOffset) {
                memoryOffset += ARRAY_OBJECT_INDEX_SCALE;
                page = getObjectUnsafe(memory, memoryOffset);
                pageOffset = ARRAY_BYTE_BASE_OFFSET;
            }
            setByteUnsafe(page, pageOffset, getByte3(value));
        }
    }
    ...
    public void putLong(int index, long value) {
        ...
        if (memoryOffset == lastByteMemoryOffset) {
            ...
            setByteUnsafe(page, pageOffset++, getByte4(value));
            if (pageOffset > lastPageOffset) {
                memoryOffset += ARRAY_OBJECT_INDEX_SCALE;
                page = getObjectUnsafe(memory, memoryOffset);
                pageOffset = ARRAY_BYTE_BASE_OFFSET;
            }
            setByteUnsafe(page, pageOffset++, getByte5(value));
            if (pageOffset > lastPageOffset) {
                memoryOffset += ARRAY_OBJECT_INDEX_SCALE;
                page = getObjectUnsafe(memory, memoryOffset);
                pageOffset = ARRAY_BYTE_BASE_OFFSET;
            }
            setByteUnsafe(page, pageOffset++, getByte6(value));
            if (pageOffset > lastPageOffset) {
                memoryOffset += ARRAY_OBJECT_INDEX_SCALE;
                page = getObjectUnsafe(memory, memoryOffset);
                pageOffset = ARRAY_BYTE_BASE_OFFSET;
            }
            setByteUnsafe(page, pageOffset, getByte7(value));
        }
    }
}
